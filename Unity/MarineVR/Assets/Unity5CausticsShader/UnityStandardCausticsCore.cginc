...
)
{
	...
	float depthDarkness = min(1.0f, s.posWorld.y / 5.0f);

	outDiffuse = half4(s.diffColor * depthDarkness, occlusion);
	outSpecSmoothness = half4(s.specColor * depthDarkness, s.oneMinusRoughness);
	outNormal = half4(s.normalWorld*0.5 + 0.5, 1);
	outEmission = half4(color * depthDarkness, 1);
}
